...
def master_service():
    try:
        ...
        app.logger.info("请求体解析成功，获得待查询数据，nprobe和topk,所用时间,logger："+str(end_query_analysis_time-start_query_analysis_time))
        ...
        app.logger.info("获取所有的请求数据，并生成二维数组成功，所用时间logger："+str(end_get_dif_query_data_time-start_get_dif_query_data_time))
        ...
        app.logger.info("计算待查询向量与聚类中心的距离成功，所用时间,logger："+str(end_calculate_distance_time-start_calculate_distance_time))
        ...
        for item in list_dis_sorted:
            ...
            try:
                slave_return_json = requests.get("http://%s:5051/faiss_service/?top_n=20&data=%s"%(slave_ip,data_string))
            ...
            app.logger.info("add slave data time::"+str(end_add_slave_result_data_time-start_add_slave_result_data_time))
        ...
        app.logger.info("获取从节点信息，并且发送请求给从节点，并接收返回信息存储为list,所用时间,logger："+str(end_query_slave_time-start_query_slave_time))
